...
export default function FormUpdate({post:{id , title , content , author}}:Props) {
  ...
  return (
    <form onSubmit={handleSubmit(HandleSubmit)} className="w-full mt-8 sm:mt-5 text-sm text-slate-600 font-light">
        ...
        <input style={errors.title && {"borderColor":"rgb(248, 113, 113)"}} {...register("title" , {required:"* required field !"})}  id="title" type="text" autoComplete="off" className=" w-full px-4 p-2 border border-slate-300 bg-slate-50 my-3 rounded-sm outline-none focus:border-green-500 transition duration-300" />
        {errors.title && <p className='text-xs text-red-600 my-3  italic'>{errors.title.message}</p>}

        <label htmlFor="content">* Content</label>
        <textarea style={errors.content && {"borderColor":"rgb(248, 113, 113)"}} {...register("content" , {required:"* required field !"})}  id="content" rows={7} className=" w-full p-8 border border-slate-300 bg-slate-50 my-3 rounded-sm outline-none focus:border-green-500 transition duration-300"></textarea>
        {errors.content && <p className='text-xs text-red-600 my-3  italic'>{errors.content.message}</p>}

        <label htmlFor="author">* Author</label>
        <input  style={errors.author && {"borderColor":"rgb(248, 113, 113)"}} {...register("author" , {required:"* required field !"})} id="author" type="text" autoCapitalize="off" autoComplete='off' className=" w-full px-4 p-2 border border-slate-300 bg-slate-50 my-3 rounded-sm outline-none focus:border-green-500 transition duration-300" />
        {errors.author && <p className='text-xs text-red-600 my-3  italic'>{errors.author.message}</p>}
        ...
        <button className="w-full p-3 bg-slate-500 text-slate-50 uppercase font-semibold tracking-wider mt-3 rounded-sm hover:bg-slate-800 hover:text-white transition duration-300 active:bg-slate-300 active:text-slate-800">Update Post</button>}
        ...
        {Result?.behavior === 'OK' && <div className='absolute z-10 top-0 left-0 flex justify-center items-center w-full bottom-0 bg-black/20'>
          <div className='w-full relative pop-up sm:w-[80%] h-fit p-5 bg-white rounded-lg'>
            <h1 className='ml-3 text-black text-lg'>Updated Successfully.</h1>
        ...
        {Result?.behavior === 'NOT_OK' && <div className='absolute z-10 top-0 left-0 flex justify-center items-center w-full bottom-0 bg-black/20'>
          ...
          <button onClick={HandleCloseClose} type='button' className='absolute top-2 right-3 text-2xl text-white hover:text-black hover:scale-105 focus:scale-95 transition-all duration-300'><IoIosClose/></button>
          </div>
        </div>}
